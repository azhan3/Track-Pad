...
class MainWindow:
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if lmList is not None:
                ...
                self.ExecutedFunction = detector.CheckAction(
                    self.img2, self.paused)
            ...
            if not self.paused:
                ...
                cv.putText(self.img2, "Paused", (10, 450),
                           cv.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 0), 3, cv.LINE_AA)
                cv.putText(self.img2, "Paused", (10, 450),
                           cv.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 2, cv.LINE_AA)
            if time.time() - config.SwipeTime < 2:
                ...
                cv.putText(self.img2, str(round(time.time() - config.SwipeTime, 1)), (10, 100), cv.FONT_HERSHEY_SIMPLEX, 3,
                           ...
                           2, cv.LINE_AA)
            self.Image()
            cv.putText(self.ImageShown, fps_text, (1280 - 250, 50),
    ...
    def Image(self):
        ...
        (thresh, self.blackAndWhiteImage) = cv.threshold(
            grayImage, 255, 255, cv.THRESH_BINARY)

        cv.circle(self.blackAndWhiteImage, (1280 - self.x1 +
                  20, self.y1), 5, (255, 255, 255), -1)
